...
from utils.CardIdEnum import CardId
from utils.CardRegistry import load_card_data
...
def get_card_from_uniqueId(move: BasicMove, game_state: GameState) -> UniqueCard | None:
...
def get_cardId_from_uniqueId(move: BasicMove, game_state: GameState) -> int:
    card = get_card_from_uniqueId(move, game_state)
    return CARD_NAME_TO_ID.get(card.name, NULL_CARD_ID) if card else None