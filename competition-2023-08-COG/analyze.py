...
summary = compile(r"^.*?'(?P<player1>.*?)' '(?P<player2>.*?)'.*?draws: (?P<draw>\d).*?P1 wins: (?P<win1>\d).*?other factors: (?P<errs>\d).*?$")
...
def score(stats):
  ...
  for player1, results in sorted(players.items(), key=order, reverse=True):
    score_print(f'{player1:>20}', results)
